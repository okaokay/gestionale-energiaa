...
async function inserisciClientiEContratti() {
    try {
        ...
        const createdBy = (userResult.rows[0] as any)?.id;
        ...
        for (let i = 0; i < clientiDemo.length; i++) {
            ...
            await pool.query(`
                INSERT INTO clienti_privati (
                    ...
                    $13, $14, $15, $16, $17, true, true, NOW(), $18, NOW()
                )
            `, [
            ...
            await pool.query(`
                INSERT INTO contratti_luce (
                    ...
                    stato, created_by, created_at
                ) VALUES (
                    $1, $2, 'privato', $3, $4, $5, $6, $7, $8, 'attivo', $9, NOW()
                )
            `, [
        ...
        const verificaClienti = await pool.query('SELECT COUNT(*) as count FROM clienti_privati');
        const verificaLuce = await pool.query('SELECT COUNT(*) as count FROM contratti_luce WHERE stato = \'attivo\'');
        const verificaGas = await pool.query('SELECT COUNT(*) as count FROM contratti_gas WHERE stato = \'attivo\'');
        
        console.log('\n🔍 Verifica database:');
        console.log(`   • Clienti privati totali: ${(verificaClienti.rows[0] as any).count}`);
        console.log(`   • Contratti luce attivi: ${(verificaLuce.rows[0] as any).count}`);
        console.log(`   • Contratti gas attivi: ${(verificaGas.rows[0] as any).count}`);
...
export { inserisciClientiEContratti };